...
- Every table header has an **icon** slot (a quiet dashed square until you set one). Click it → a searchable picker of curated monochrome line icons, grouped by theme, with an **Upload image…** option for your own (downscaled to 64px, stored as a data URL). Picked on click, saved immediately to `localStorage`, and the icon then shows in the header and the table directory. Generic — the picker (`SakuraStudy.iconPicker`) and store (`SakuraStudy.tableCustom`) are reusable anywhere a table id is known
