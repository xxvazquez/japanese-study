...
- Every table header has an **icon** slot (a quiet dashed square until you set one). Click it → a searchable picker of curated monochrome line icons, grouped by theme, with an **Upload image…** option for your own (downscaled to 64px, stored as a data URL). Picked on click, saved immediately. Storage follows the same model as flashcards: `localStorage` is the immediate source of truth (headers render without waiting on the network), and while you're signed in it also syncs through your account (a `table_custom` column on `flashcard_settings`), so a header icon set on your phone shows up on the web too. Generic — the picker (`SakuraStudy.iconPicker`) and store (`SakuraStudy.tableCustom`) are reusable anywhere a table id is known
