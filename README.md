...
- `js/vocab/` — the vocabulary page: `kana-romaji.js` (`SakuraStudy.kanaRomaji` — kana→romaji converter for the hover/tap reading layer), `icons.js` (`SakuraStudy.icons` — a curated monochrome line-icon set, a Lucide subset), `table-custom.js` (`SakuraStudy.tableCustom` — per-table personalisation in `localStorage`), `icon-picker.js` (`SakuraStudy.iconPicker` — the reusable picker), `render.js` (tables + the four-item navigation, sorting), `interactions.js` (section routing, search, view modes, menus, print, polite toggle, theme, reading-layer taps, icon picking)
...
- Every table header has an **icon** slot (a quiet dashed square until you set one). Click it → a searchable picker of ~165 curated monochrome line icons (a Lucide subset) in five themed groups — Food & drink, Travel & places, Home & objects, Nature & weather, Symbols & UI — with an **Upload image…** option for your own (downscaled to 64px, stored as a data URL). Picked on click, saved immediately. Storage follows the same model as flashcards: `localStorage` is the immediate source of truth (headers render without waiting on the network), and while you're signed in it also syncs through your account (a `table_custom` column on `flashcard_settings`), so a header icon set on your phone shows up on the web too. Generic — the picker (`SakuraStudy.iconPicker`) and store (`SakuraStudy.tableCustom`) are reusable anywhere a table id is known
